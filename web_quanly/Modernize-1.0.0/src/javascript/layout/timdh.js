...
async function getDataTim() {
    try {
        const search_sdt = document.getElementById("search_sdt").value;
        const response = await fetch(`http://localhost:3000/donhang/find?sdt_nguoigui=${search_sdt}`);
...
function fomatDate(date) {
    ...
    var year = newDate.getFullYear();// lay nam
    var month = ("0" + (newDate.getMonth() + 1)).slice(-2);//lay thang
    var day = ("0" + newDate.getDate()).slice(-2);//lay ngay
